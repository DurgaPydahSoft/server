/**
 * One-time migration: set all active hostel students to a target academic year.
 *
 * Usage:
 *   node -r dotenv/config src/scripts/migrateActiveStudentsAcademicYear.js
 *   node -r dotenv/config src/scripts/migrateActiveStudentsAcademicYear.js 2026-2027
 */
...
const TARGET_AY = process.argv[2] || '2026-2027';
...
const run = async () => {
  ...
  console.log(`Migrating active students to academic year: ${TARGET_AY}`);

  const activeStudents = await User.find({
    role: 'student',
    hostelStatus: 'Active'
  }).select('_id rollNumber name academicYear applicationStatus');
  ...
  const alreadyCorrect = activeStudents.filter((s) => s.academicYear === TARGET_AY).length;
  ...
  const feeResult = await FeeReminder.updateMany(
    { student: { $in: studentIds }, isActive: true },
    { $set: { academicYear: TARGET_AY } }
  );

  const historyResult = await RoomOccupancyHistory.updateMany(
    { student: { $in: studentIds }, status: 'Active', allocatedTo: null },
    { $set: { academicYear: TARGET_AY } }
  );
  ...
  console.log(`Active fee reminders updated: ${feeResult.modifiedCount}`);
  console.log(`Active occupancy history rows updated: ${historyResult.modifiedCount}`);
  console.log('Done.');
};
